...
public class LocalNavMeshBuilder : MonoBehaviour
{
   ...
   public static List<NavMeshBuildSource> m_Sources = new List<NavMeshBuildSource>();
    ...
    public IEnumerator myStart()
    {
            ...
            Debug.Log("Started NavmeshBuidler");
            while (true)
            {
                UpdateNavMesh(ASYNC);
    ...
    void UpdateNavMesh(bool asyncUpdate)
    {
        if (NavMeshSourceTag.Changed)
        {
            ...
            defaultBuildSettings.agentRadius = agentSize;
            defaultBuildSettings.maxJobWorkers = 1;
            
            if (asyncUpdate)
                m_Operation = NavMeshBuilder.UpdateNavMeshDataAsync(m_NavMesh, defaultBuildSettings, m_Sources, bounds);
            else
                NavMeshBuilder.UpdateNavMeshData(m_NavMesh, defaultBuildSettings, m_Sources, bounds);
            NavMeshSourceTag.Changed = false;
