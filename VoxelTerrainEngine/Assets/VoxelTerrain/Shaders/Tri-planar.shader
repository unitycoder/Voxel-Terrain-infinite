...
Shader "Triplanar/Surface Shader (RNM)" {
    Properties{
        ...
        NormalInt("Normal Intensity", Range(0, 1)) = 0.5
        ...
        SubShader{
            ...
            float4 _BumpMap_ST, _BumpMap2_ST;
            sampler2D _OcclusionMap;
            half _ScrollSpeed;
            half _Glossiness, NormalInt;
            half _Metallic;
            ...
            void surf(Input IN, inout SurfaceOutputStandard o) {
                ...
                half3 tnormalX1 = UnpackNormal(tex2D(_BumpMap2, (uvX * _BumpMap2_ST.xy) + float2(-_Time.x, _Time.x)*2 * _ScrollSpeed))* NormalInt;
                half3 tnormalY1 = UnpackNormal(tex2D(_BumpMap2, (uvY * _BumpMap2_ST.xy) + float2(-_Time.x, _Time.x) * 2 * _ScrollSpeed)) * NormalInt;
                half3 tnormalZ1 = UnpackNormal(tex2D(_BumpMap2, (uvZ * _BumpMap2_ST.xy) + float2(-_Time.x, _Time.x) * 2 * _ScrollSpeed)) * NormalInt;

                half3 tnormalX = UnpackNormal(tex2D(_BumpMap, (uvX * _BumpMap_ST.xy) + float2( _Time.x, _Time.x) * _ScrollSpeed + tnormalX1.xy));
                half3 tnormalY = UnpackNormal(tex2D(_BumpMap, (uvY * _BumpMap_ST.xy) + float2(_Time.x, _Time.x) * _ScrollSpeed + tnormalY1.xy));
                half3 tnormalZ = UnpackNormal(tex2D(_BumpMap, (uvZ * _BumpMap_ST.xy)+ float2(_Time.x, _Time.x) * _ScrollSpeed + tnormalZ1.xy));
